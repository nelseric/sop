...
public:
  ...
  AbstSoccerTeam*          m_pRedTeam;
  AbstSoccerTeam*          m_pBlueTeam;
  ...
  // 1/20/rb Added for competition
  time_t				m_startTime;
  LARGE_INTEGER				m_redTotalTime;
  LARGE_INTEGER				m_blueTotalTime;
  bool					m_blueReady;
  bool					m_redReady;
...
public:
	...
	SoccerPitch(int cxClient, int cyClient, std::string redName, std::string blueName);
  ...
  void	SetRedTeamReady ();
  void	SetBlueTeamReady ();
  bool	OneTeamReady () { return m_redReady || m_blueReady; }
